		...
		.on('end', function(){
			...
			bayesTypes.forEach(function (bayesType) {
				...
				file.categories.forEach(function (primaryCategory) {
					...
					var start = 0;
					var middle, end;
					var chunkLength = dataSet.length/10;
					for (var i = 0; i < 10; i++)
					{
						...
						middle = start + chunkLength;
						end = middle + chunkLength;

						// repeat for as long as our categories are empty
						do {
							dataSet = testFunctions.shuffle(dataSet);

							sets = testFunctions.partitionSets(dataSet, start, middle, end);
							// get all categories
						...
						sets.testingSet.forEach(function(dataPoint) {
							...
							if (bayesType === 'linear') classifier.classifyLinear(dataPoint);
				...
				winston.info('Algorithm: %s, Average percent: %d%', bayesType,file.results[bayesType]/file.categories.length/10);
			});
